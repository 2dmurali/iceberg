...
public class TestIncrementalScanPlanningAndReporting extends TestBase {
  ...
  @TestTemplate
  public void incrementalChangelogScanEmitsScanReport() throws IOException {
    String tableName = "incremental-changelog-scan-report";
    Table table =
        TestTables.create(
            tableDir, tableName, SCHEMA, SPEC, SortOrder.unsorted(), formatVersion, reporter);
    ...
    IncrementalChangelogScan scan =
        table.newIncrementalChangelogScan().fromSnapshotExclusive(snapshotId1);
    ...
    ScanMetricsResult result = scanReport.scanMetrics();
    assertThat(result.totalPlanningDuration().totalDuration()).isGreaterThan(Duration.ZERO);
    // resultDataFiles and totalFileSizeInBytes are populated by ManifestGroup.createFileScanTasks,
    // which only the append scan uses. Changelog scans build tasks via CreateDataFileChangeTasks,
    // so those counters stay at 0; manifest counters still reflect what was scanned.
    assertThat(result.totalDataManifests().value()).isEqualTo(1);
    assertThat(result.scannedDataManifests().value()).isEqualTo(1);
    assertThat(result.skippedDataManifests().value()).isEqualTo(0);
  }
